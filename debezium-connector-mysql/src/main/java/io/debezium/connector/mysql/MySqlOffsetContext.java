...
public class MySqlOffsetContext implements OffsetContext {
    ...
    public MySqlOffsetContext(MySqlConnectorConfig connectorConfig, boolean snapshot, boolean snapshotCompleted, SourceInfo sourceInfo) {
        this(connectorConfig, snapshot, snapshotCompleted, new TransactionContext(),
                connectorConfig.isReadOnlyConnection() ? new ReadOnlyMySqlIncrementalSnapshotContext<>() : new SignalBasedIncrementalSnapshotContext<>(),
                sourceInfo);
    }
    ...
    public static class Loader implements OffsetContext.Loader<MySqlOffsetContext> {
        ...
        public MySqlOffsetContext load(Map<String, ?> offset) {
            ...
            if (connectorConfig.isReadOnlyConnection()) {
                incrementalSnapshotContext = ReadOnlyMySqlIncrementalSnapshotContext.load(offset);
            }
            else {
