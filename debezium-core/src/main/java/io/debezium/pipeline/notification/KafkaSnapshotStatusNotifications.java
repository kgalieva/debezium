...
public class KafkaSnapshotStatusNotifications<T extends DataCollectionId> implements SnapshotStatusNotifications<T> {
    ...
    public static Field.Set ALL_FIELDS = Field.setOf(TOPIC, BOOTSTRAP_SERVERS, SnapshotStatusNotifications.NAME);
    ...
    public void configure(Configuration config) {
        ...
        String nofiticationsName = config.getString(SnapshotStatusNotifications.NAME, UUID.randomUUID().toString());
        this.producerConfig = config.subset(PRODUCER_PREFIX, true).edit()
                ...
                .withDefault(ProducerConfig.CLIENT_ID_CONFIG, nofiticationsName)
    ...
    private Document toDocument(DataCollection<T> dataCollection, SnapshotCompletionStatus status) {
        ...
        document.setString("type", "incremental-snapshot");
        document.setString("data-collection", dataCollection.getId().identifier());
        dataCollection.getAdditionalCondition().ifPresent(s -> document.setString("additional-condition", s));
